...
userrouter.patch("/:userId", userAuth, async (req, res) => {
  ...
  try {
    ...
    const updates = Object.keys(updatedUserData);
...
userrouter.get("/requests/received", userAuth, async (req, res) => {
  try {
    ...
    const requests = await ConnectionRequest.find({
      ...
      "senderId",
      "firstname lastname email age gender skills photoURL about"
    );
...
userrouter.get("/connection", userAuth, async (req, res) => {
  try {
    ...
    if (requests.length === 0) {
      return res
        .status(404)
        .json({ message: "No accepted requests found" });
    }

    const data = requests.map((request) => ({
      connectionWith: request.senderId._id.equals(userId)
        ? request.receiverId
        : request.senderId,
